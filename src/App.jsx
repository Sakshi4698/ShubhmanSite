...
const BRANDS = [
  ...
  {name:"TASVA",                 color:"#444",   abbr:"TASVA"},
            ...
            <div className="portrait-flip-wrap" style={{ width: 220, height: 220, borderRadius: "50%", border: `3px solid ${C.gold}`, boxShadow: "0 0 0 5px rgba(212,175,55,.15),0 0 40px rgba(212,175,55,.2)", position: "relative", zIndex: 2, perspective: 900, cursor: "pointer" }}>
              <div className="portrait-flip-inner">
                <div className="portrait-flip-front">
                  <img src={asset("assets/hero.png")} alt="Shubman Gill" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 20%", display: "block" }} />
                </div>
                <div className="portrait-flip-back">
                  <img src={asset("assets/hero.png")} alt="Shubman Gill BW" style={{ width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 20%", display: "block" }} />
                </div>
              </div>
            </div>
...
function TiltCard() {
  ...
  return (
    <div ref={ref} onMouseMove={isMobile ? undefined : onMove} onMouseLeave={isMobile ? undefined : onLeave}
      ...
      <img src={asset("assets/short_cover.jpg")} alt="Signature Shot"
        style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center 30%", display: "block", transform: "scale(1.05)" }} />
...
function BrandCard({ brand }) {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      <img src={asset(`assets/brands/${brand.name.toLowerCase().replace(/ /g, "-")}.png`)} alt={brand.name}
...
// ── Socials ───────────────────────────────────────────────────────────────────
function SocialCard({ s, i, transform, zIndex, opacity, onEnter, onLeave, onTouchEnd }) {
  const { isMobile } = useBreakpoint();
  const w = isMobile ? 160 : 200, h = isMobile ? 220 : 280;
  return (
    <div onMouseEnter={onEnter} onMouseLeave={onLeave} onTouchEnd={onTouchEnd}
      ...
      <img src={asset(`assets/social-${i + 1}.jpg`)} alt={s.name}
        style={{ position: "absolute", inset: 0, width: "100%", height: "100%", objectFit: "cover", objectPosition: "center", display: "block", filter: "brightness(.85)" }} />
...
function SocialsSection() {
  ...
  const fanned = isMobile ? touched : deckHovered;
  ...
  return (
    <section id="socials" style={{ padding: isMobile ? "60px 20px" : "100px 40px", background: `linear-gradient(to bottom,#040f24,${C.dark})` }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <p style={{ fontSize: ".8rem", color: "rgba(240,244,255,.4)", marginBottom: isMobile ? 80 : 100, letterSpacing: 1 }}>
          {isMobile ? "Tap to fan · Tap a card to highlight" : "Hover to fan · Click to visit"}
        </p>
        <div style={{ position: "relative", height: isMobile ? 280 : 360, display: "flex", justifyContent: "center", alignItems: "center" }}
          onMouseEnter={() => !isMobile && setDeckHovered(true)}
          onMouseLeave={() => { if (!isMobile) { setDeckHovered(false); setHoveredIdx(-1); } }}
          onClick={() => isMobile && setTouched(t => !t)}>
          {SOCIALS.map((s, i) => (
            <SocialCard key={s.name} s={s} i={i}
              transform={getTransform(i)}
              zIndex={hoveredIdx === i ? 100 : i + 10}
              opacity={fanned && hoveredIdx >= 0 && hoveredIdx !== i ? 0.7 : 1}
              onEnter={() => { if (!isMobile) { setDeckHovered(true); setHoveredIdx(i); } }}
              onLeave={() => { if (!isMobile) setHoveredIdx(-1); }}
              onTouchEnd={(e) => { e.stopPropagation(); handleCardTouch(i); }}
            />
          ))}
        </div>
        {isMobile && (
          <p style={{ textAlign: "center", fontSize: ".65rem", color: "rgba(240,244,255,.25)", marginTop: 16, letterSpacing: 2 }}>
            {touched ? "TAP A CARD TO VISIT" : "TAP TO EXPAND"}
          </p>
        )}
      </div>
    </section>
...
function Footer() {
  ...
  const toggleSound = () => {
    if (!audioRef.current) {
      audioRef.current = new Audio(asset("assets/crowd-cheer.mp3"));
